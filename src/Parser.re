let rec parse_factor = (tokenList): (Ast.factor, list(Token.token)) => {
  ...
  if (token
    ...
    if (Ast.identi_factor(fac) == true) {
      (
        /*Revisa que el nodo recibido sea correcto*/
        Ast.UnOp(token, fac),
        remainingTokens /*Devolvemos el nodo*/
      );
    } else {
      (
    ...
    /*En caso de que el token no sea una operacion unaria, debe de ser una constante*/
    (token |> Token.identificador == "Constant") {
    /*Comprobamos que el token sea una constante*/
    ...
    (
      Ast.Err_fac("Error, falta una constante u operador unario"),
      tokenList /*En caso de error, devuelve un nodo error*/
    );
  };
};
...
let parse_termino = (tokenList): (Ast.termino, list(Token.token)) => {
  ...
  if (Ast.identi_factor(fac1) == true) {
    /*Revisa que el nodo recibido sea correcto*/
    let [token, ...r] = tokenList; /*Hacemos un pop a la lista*/
    if (token
        |> Token.identificador == "Multiplication"
        || token
        |> Token.identificador == "Division") {
      /*Revisa si es alguno de los operadores * o / */
      ...
      if (Ast.identi_factor(fac2) == true) {
        (
          /*Revisa que el nodo recibido sea correcto*/
          Ast.BinOp(oper, fac1, fac2),
          r /*Devuelve el nodo de la exp del tipo operacion binaria*/
        );
      } else {
        (
          Ast.Err_ter(Ast.ext_fac_err(fac1)),
          r /*En caso de que el nodo que nos devolvió el parse_factor este mal devuelve un nodo error*/
        );
      };
    } else {
      (
        Ast.Ter(fac1),
        tokenList /*En caso de que el token que seguía del primer termino no sea un operador, devuelve unicamente el factor 1*/
      );
    };
  } else {
    (
      Ast.Err_ter(Ast.ext_fac_err(fac1)),
      tokenList /*En caso de que el nodo que nos devolvió el parse_factor este mal devuelve un nodo error*/
    );
  };
};
...
let parse_exp = (tokenList): (Ast.exp, list(Token.token)) => {
  ...
  if (Ast.identi_termino(ter1) == true) {
    /*Revisa que el nodo recibido sea correcto*/
    let [token, ...r] = tokenList; /*Hacemos un pop a la lista*/
    if (token
        |> Token.identificador == "Addition"
        || token
        |> Token.identificador == "Negation") {
      /*Revisa si es alguno de los operadores + o -*/
      let oper = token; /*Guarda el token de la operación en una variable*/
      ...
      if (Ast.identi_termino(ter2) == true) {
        (
          /*Revisa que el nodo recibido sea correcto*/
          Ast.BinOp(oper, ter1, ter2),
          r /*Devuelve el nodo de la exp del tipo operacion binaria*/
        );
      } else {
        (
          Ast.Err_exp(Ast.ext_ter_err(ter2)),
          r /*En caso de que el nodo que nos devolvió el parse_termino este mal devuelve un nodo error*/
        );
      };
    } else {
      (
        Ast.Expr(ter1),
        tokenList /*En caso de que el token que seguía del primer termino no sea un operador, devuelve unicamente el termino 1*/
      );
    };
  } else {
    (
      Ast.Err_exp(Ast.ext_ter_err(ter1)),
      tokenList /*En caso de que el nodo que nos devolvió el parse_termino este mal devuelve un nodo error*/
    );
  };
};

let parse_statement = (tokenList): (Ast.statement, list(Token.token)) => {
  /*Recibe la lista de token y devueleve el token y la lista modificada*/
  let [token, ...remainingTokens] = tokenList; /*Sacamos siguiente token que debe de ser el return*/
  if (token |> Token.identificador == "ReturnKeyword") {
    /*Comprobamos que el token sea return*/
    ...
    if (Ast.identi_exp(exp) == true) {
      /*Revisa que el nodo recibido sea correcto*/
      let [token, ...remainingTokens] = remainingTokens; /*Sacamos siguiente token que debe de ser ";"*/
      if (token |> Token.identificador == "Semicolon") {
        (
          /*Revisa que efectivamente sea el ; que venga*/
          Ast.Assign(var, exp),
          remainingTokens /*Devuelve el nodo junto con lo que queda de la lista*/
        );
      } else {
        (
          Ast.Err_state("Error, falta ;"),
          remainingTokens /*En caso de que falte el ; devolver el nodo error*/
        );
      };
    } else {
      (
        Ast.Err_state(Ast.ext_exp_err(exp)),
        remainingTokens /*En caso de que el nodo que nos devolvió el parse_exp este mal devuelve un nodo error*/
      );
    };
  } else {
    (
      Ast.Err_state("Error, falta return"),
      remainingTokens /*En caso de que el token que viene no sea el return se devuelve un nodo error*/
    );
  };
};
...
let parse_function = (tokenList): Ast.fun_decl => {
  ...
  if (token |> Token.identificador == "IntKeyword") {
    /*Comprobamos que el token sea Intkeyword*/
    let [token, ...remainingTokens] = remainingTokens; /*Sacamos el siguiente elemento de la lista que será un id*/
    if (token |> Token.identificador == "Id") {
      /*Comprobamos que el token sea un Id*/
      ...
      if (token |> Token.identificador == "OpenParen") {
        /*Comprobamos que el token sea un (*/
        ...
        if (token |> Token.identificador == "CloseParen") {
          ...
          if (token |> Token.identificador == "OpenBrace") {
            ...
            if (Ast.identi_statement(return) == true) {
              /*Revisamos que el nodo devuelto no tenga error*/
              let [token, ...remainingTokens] = remainingTokens; /*Sacamos el siguiente elemento de la lista que debe de ser }*/
              if (token |> Token.identificador == "CloseBrace") {
                /*En teoría debe de revisar que lo que sigue es el "}"*/
                Ast.Fun
                  (idName, return); /*Devuelve el nodo de funcion*/
              } else {
                Ast.Err_fun
                  ("Error, falta }"); /*En caso de que falte }, se crea un nodo error y se devuelve*/
              };
            } else {
              Ast.Err_fun
                (Ast.ext_state_err(return)); /*En caso de que el nodo devuelto por el parse_statement tenga error, crea otro nodo error que será devuelto*/
            };
          } else {
            /*Lo demás nos nodos error en caso de que se presente alguno*/
            Ast.Err_fun(
              "Error, falta {",
            );
          };
...
let parse_program = (tokenList): Ast.prog => {
  ...
  if (Ast.identi_fun_decl(func_decl) == true) {
    /*Revisamos que el nodo devuelto no sea de un error*/
    Ast.Prog
      (func_decl); /*Devuelve el AST de manera correcta*/
  } else {
