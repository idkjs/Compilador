let rec generate_factor = (ast: Ast.factor, reg: int) =>
  ...
  | Const(value) =>
    ...
    let return_statement =
      "    movl    " ++ decimal ++ ", " ++ Words.registro(reg);
  ...
  | UnOp(ope, factor) =>
    if (ope |> Token.identificador == "LogNeg") {
      ...
      let return_unop =
        "    "
        ++ Words.tradu(ope |> Token.identificador)
        ++ "    "
        ++ Words.registro(1);
      generate_factor(factor, 0);
      Js.log(return_unop);
...
let generate_exp = (ast_exp: Ast.exp) =>
  ...
  | Ast.BinOp(ope, ter1, ter2) =>
    ...
    let return_exp =
      "    add    " ++ Words.registro(0) ++ ", " ++ Words.registro(1);
    Js.log(return_exp);
  | Ast.Expr(ter) => generate_termino(ter)
  };
