...
let tokenToString = (acummulator, t: token): string => {
  /*Identificamos los token para imprimir la lista*/
  let result =
    switch (t) {
    | OpenBrace => "OpenBrace"
...
let tokenListToString = tokensList => {
  /*Convierte la lista de token en string*/
  let comma_regexp = [%re "/,$/"];
...
let printTokenList = tokenList => {
  /*Imprime la lista de token*/
  tokenList |> tokenListToString |> Js.log;
  tokenList;
};

let identificador = (t: token) => {
  /*Para identificar los token en la comparación*/
  let result =
    switch (t) {
    | OpenBrace => "OpenBrace"
    | CloseBrace => "CloseBrace"
...
let extrac = (t: token) => {
  /*Extraemos los valores que tengan el id y/o constant*/
  let result =
    switch (t) {
    | Id(n) => n
